...
- [ ] GSCドメインプロパティ `sc-domain:digitool-lab.com` で、`digitool-lab.com` / `showroom.digitool-lab.com` / `plat.digitool-lab.com` / `shop.digitool-lab.com` を横断して監視する
  ...
  - 2026-06-24: `data/gsc/` は検索クエリを含むため公開URLでは `RedirectMatch 410` と `RewriteRule [G]` で410、FTPデプロイ対象から除外、gitでは `.gitkeep` 以外をignoreし、`scripts/audit-private-seo-data.mjs` でCI監査するようにした。
