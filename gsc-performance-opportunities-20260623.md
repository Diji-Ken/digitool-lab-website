...
## 2026-06-24 Gemini/GAS系流入ページの相談導線補強

GSC上位ページに `/blog/ai-gemini-gas-file-rename` が32クリック/258表示で出ており、`gemini スクリプト` も検索クエリとして表示されているため、情報収集流入を業務自動化・業務システム相談へ接続した。

- `/blog/ai-gemini-gas-file-rename` に、GAS自動化から業務システム、社内ポータル、無料チェックリストへ進める関連サービス導線を追加した
- `/blog/ai-gemini-file-rename` に、ファイル整理から業務システム、社内ポータル、DX伴走支援へ進める関連サービス導線を追加した
- `/blog/ai-gemini-script-review` の関連サービス導線を、社内ポータルから `業務システム開発` へ寄せ、GAS/Excelマクロ改善後の保守しやすいシステム化へ接続した
- 3記事の `dateModified` と sitemap `lastmod` を2026-06-24に更新した
- 次回GSCでは、`gemini スクリプト`、`GAS ファイル名 変更`、`Gemini GAS`、`Google Drive ファイル名 一括変更` のクリック、表示、平均順位と、相談導線クリックを確認する
